...
public static class HidMaestroBridge {
    ...
    sealed class TrayHost:NativeWindow,IDisposable {
        const int WM_HOTKEY=0x0312,WM_STATUS=0x8001,HOTKEY_ID=0x5443,MOD_NOREPEAT=0x4000;
        ...
        readonly Action quit;readonly NotifyIcon tray;readonly ToolStripMenuItem toggleItem;volatile bool enabled;volatile bool routing;
        public bool Enabled=>enabled;
        static uint ToggleVirtualKey(string value){
            string key=(value??"").Trim().ToUpperInvariant();
            if(key.Length==1&&key[0]>='A'&&key[0]<='Z')return key[0];
            if(key.Length>=2&&key[0]=='F'&&int.TryParse(key.Substring(1),out int function)&&function>=1&&function<=24)return (uint)(0x6f+function);
            return key switch {"SPACE"=>0x20,"TAB"=>0x09,"ESC"=>0x1b,"ESCAPE"=>0x1b,"PAUSE"=>0x13,"CAPSLOCK"=>0x14,"INSERT"=>0x2d,"DELETE"=>0x2e,"HOME"=>0x24,"END"=>0x23,"PAGEUP"=>0x21,"PAGEDOWN"=>0x22,_=>throw new ArgumentException("ToggleKey must be a letter, F1 through F24, or a supported named key (Space, Tab, Escape, Pause, CapsLock, Insert, Delete, Home, End, PageUp, PageDown). Fn is not a Windows key and cannot be registered.")};
        }
        public TrayHost(Action quit,bool startEnabled,string toggleKey){this.quit=quit;enabled=startEnabled;CreateHandle(new CreateParams{Caption="TrackPad CAD hotkey"});
            if(!RegisterHotKey(Handle,HOTKEY_ID,MOD_NOREPEAT,ToggleVirtualKey(toggleKey)))throw new InvalidOperationException(toggleKey+" is already registered by another app.");
            ...
            tray=new NotifyIcon{ContextMenuStrip=menu,Visible=true,Text="TrackPad CAD — disabled",Icon=MakeIcon(Color.FromArgb(115,115,115))};tray.DoubleClick+=(s,e)=>Toggle();UpdateUi();}
        static Icon MakeIcon(Color color){using var bmp=new Bitmap(32,32);using(var g=Graphics.FromImage(bmp)){g.SmoothingMode=System.Drawing.Drawing2D.SmoothingMode.AntiAlias;g.Clear(Color.Transparent);using var b=new SolidBrush(color);using var p=new Pen(Color.White,2);g.FillEllipse(b,3,3,26,26);g.DrawEllipse(p,3,3,26,26);g.DrawLine(p,10,16,22,16);g.DrawLine(p,16,10,16,22);}IntPtr h=bmp.GetHicon();try{using var temp=Icon.FromHandle(h);return (Icon)temp.Clone();}finally{DestroyIcon(h);}}
        void Toggle(){enabled=!enabled;UpdateUi();System.Media.SystemSounds.Asterisk.Play();}
        public void SetRouting(bool value){if(routing==value)return;routing=value;PostMessage(Handle,WM_STATUS,IntPtr.Zero,IntPtr.Zero);}
        void UpdateUi(){toggleItem.Text=enabled?"Disable":"Enable";string state=!enabled?"disabled":routing?"active":"standby";tray.Text="TrackPad CAD — "+state;var old=tray.Icon;tray.Icon=MakeIcon(!enabled?Color.FromArgb(115,115,115):routing?Color.FromArgb(45,190,85):Color.FromArgb(235,190,35));old?.Dispose();}
        protected override void WndProc(ref Message m){if(m.Msg==WM_HOTKEY&&m.WParam==(IntPtr)HOTKEY_ID)Toggle();else if(m.Msg==WM_STATUS)UpdateUi();base.WndProc(ref m);}
        public void Dispose(){tray.Visible=false;tray.Dispose();UnregisterHotKey(Handle,HOTKEY_ID);DestroyHandle();}
    }

    public static void Run(HMController controller,string exe,string target,int seconds,double sensitivity,bool startEnabled,string toggleKey,string[] nativeExecutables,string[] browserUrlPrefixes){
        ...
        var ui=new Thread(()=>{try{using(host=new TrayHost(()=>{quit=true;Application.ExitThread();},startEnabled,toggleKey)){ready.Set();Application.Run();}}catch(Exception e){uiError=e;ready.Set();quit=true;}});ui.SetApartmentState(ApartmentState.STA);ui.IsBackground=true;ui.Start();ready.Wait();if(uiError!=null)throw uiError;
